...
class FunSetSuite extends FunSuite {
  ...
  test("diff is the set of all elements of `s` that are not in `t`") {
    new TestSets {
      val s = diff(union(union(s1, s2), s3), s3)
      assert(contains(s, 1), "Intersect 1")
      assert(contains(s, 2), "Intersect 2")
      assert(!contains(s, 3), "Intersect 3")
    }
  ...
  test("forall: returns whether all bounded integers within `s` satisfy `p`") {
    new TestSets {
      ...
      assert(!forall(s, x => x > 0 ), "All strictly positive")
      assert(!forall(s, x => x == 0 ), "Only zeros")
      assert(!forall(s, IsPowerOfTwo ), "Only power of two")
    }
  }
  ...
  test("exists: whether there exists a bounded integer within `s` that satisfies `p`") {
    new TestSets {
      val s = union(singletonSet(0), union(union(singletonSet(2), singletonSet(4)), singletonSet(6)))
      assert(exists(s, x => x == 0), "Exists zero")
      assert(exists(s, IsPowerOfTwo), "Exists power of two")
    }
  }

  test("map: returns a set transformed by applying `f` to each element of `s`") {
    new TestSets {
      val s = map(union(singletonSet(0), union(union(singletonSet(2), singletonSet(4)), singletonSet(6))), x => 2 * x)
      assert(!contains(s, 2), "Contains 2")
      assert(contains(s, 4), "Contains 4")
      assert(contains(s, 12), "Contains 12")
    }
  }
