...
object Huffman {
  ...
  /**
    ...
    */
  def createCodeTree(chars: List[Char]): CodeTree = until(x => singleton(x), x => combine(x))(makeOrderedLeafList(times(chars)))
  ...
  /**
    ...
    */
  def decodeSubTree(currentTree: CodeTree, bits: List[Bit], fullTree: CodeTree): List[Char] = currentTree match {
    case Leaf(c, _) => c :: decodeSubTree(fullTree, bits, fullTree)
    case Fork(l, r, cs, w) => bits match {
      case Nil => Nil
      case b :: bs => if(b == 0) decodeSubTree(l, bs, fullTree) else decodeSubTree(r, bs, fullTree)
    }
  }

  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = decodeSubTree(tree, bits, tree)
  ...
  /**
    ...
    */
  def encodeHelper(tree: CodeTree)(text: List[Char], subTree: CodeTree): List[Bit] = text match {
    case Nil => Nil
    case t :: ts => subTree match {
      case Fork(l, r, _, _) =>
        if(chars(l).contains(t))
          0 :: encodeHelper(tree)(text, l)
        else
          1 :: encodeHelper(tree)(text, r)
      case Leaf(_, _) => encodeHelper(tree)(ts, tree)
    }
  }

  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = encodeHelper(tree)(text, tree)
  ...
  def convertHelper(tree: CodeTree, chars: List[Char]): CodeTable = chars match {
    case Nil => Nil
    case c :: cs => (c, encode(tree)(List(c))) :: convertHelper(tree, cs)
  }

  def convert(tree: CodeTree): CodeTable = convertHelper(tree, chars(tree))
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
    case Nil => Nil
    case t :: ts => codeBits(convert(tree))(t) ::: quickEncode(tree)(ts)
  }
}
